...
function refresh_pong() {
    if (gameStarted == true) {
        ...
        player_1.setAttribute("style", "position:absolute; left:30px; top:" + (gamePane.offsetHeight / 2.5) +
            "px; width:10px; height:18%;  background: repeating-linear-gradient(55deg," +
            color_palette[(color_theme + 1) % 3][7] + ",#fff 5px,#aaa 2px,#465298 20px)");
        player_2.setAttribute("style", "position:absolute; right:30px; top:" + (gamePane.offsetHeight / 2.5) +
            "px; width:10px; height:18%;  background: repeating-linear-gradient(55deg," +
            color_palette[(color_theme + 1) % 3][7] + ",#fff 5px,#aaa 20px,#465298 20px)");
    }
...
function game_loop() {
    ...
    if (document.getElementById('gameSync').checked == true) {
        ...
        if (triggered[3] == 0) {
            ...
            if (slowlydrag > 5) {
                ...
                if (mouse.y > parseInt((gamePaneHeight.offsetHeight / 2) + parseInt(gamePaneHeight.offsetTop))) {
                    mouse.y = mouse.y - 1;
                }
                if (mouse.y < parseInt((gamePaneHeight.offsetHeight / 2) + (gamePaneHeight.offsetTop))) {
                    mouse.y = mouse.y + 1;
                }
                slowlydrag = 0;
            }
    ...
    if (ball == null) {
        ...
        ball.setAttribute("style",
            "position:absolute; left:" + (gamePane.offsetWidth / 2.04) + "px; top:" + (gamePane.offsetHeight / 2.1) +
            "px; width:10px;" +
            "border-radius:15px; border: 12px solid #" +
            ((color_theme * 15) + (color_theme * 15) + (color_theme * 155)));
    ...
    if (gameReady == 2) {
        ...
        if ((ball.offsetLeft + 2) < (player_1.offsetLeft + 10)) {
            ...
            if ((ball.offsetTop + 11) >= player_1.offsetTop && (ball.offsetTop + 11) <=
                (player_1.offsetTop + player_1.offsetHeight)) {
        ...
        if ((ball.offsetLeft + 18) > (player_2.offsetLeft - 2)) {
            ...
            if ((ball.offsetTop + 11) >= player_2.offsetTop && (ball.offsetTop + 11) <=
                (player_2.offsetTop + player_2.offsetHeight)) {
