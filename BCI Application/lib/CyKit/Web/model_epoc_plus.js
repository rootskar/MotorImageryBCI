...
function process_EEG_data() {
    ...
    if (contact[1] == "16") {
        if (no_battery == false && no_counter == false) {
            if (formatType == 0) {
                var battery_position = 16;
            } else {
                var battery_position = 30;
            }
            //if (contact[
            if (contact[0] == 127 || contact[0] == 255) {
                ...
                if (contact[battery_position] != 0) {
                    console.log(
                        battery_position + " " + contact[battery_position] + " " + contact[30] + " " + contact[31]);

                    //var battery_percent = (((parseInt(contact[battery_position]) - 117) +25) *3.85);
                    var battery_percent = ((parseInt(contact[battery_position]) - battery_mode) * 1.612903); //128hz
                                                                                                             // mode
                                                                                                             // ---
                    //var battery_percent = (((parseInt(contact[battery_position]) - 117) +25)  * 3.85); //      255hz
                    // mode ---  var battery_percent = ((119.5 - parseInt(contact[battery_position])) * 1.612903);
    ...
    if (contact[1] == "16") {
        ...
        if (manualControl) {
            ...
            // ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
                ...
                drawdot(myContact, 50, cy_x, (value_data - baseline[selectedContact] * eeg_resolution),
                    color_palette[color_theme][0]);
            }
        } else {
            ...
            if (formatType == 0) {
                ...
                for (arr_element in sensorNAME['epoc']) {
                    ...
                    if (c.checked == true) {
                        ...
                        if (document.getElementById("enable-singleWave").checked == true) {
                            inc_y_offset = (canvasHeight / 2);
                        }
                        drawdot(arr_element, inc_y_offset, cy_x, value_floor - ((value_data * .128)) * eeg_resolution,
                            myColor);
                    }
                }
            }
            ...
            // ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
                ...
                for (var i = 2; i < (contact.length - (no_counter == true ? 0 : 0)); i += 2) {
                    ...
                    if (c.checked == true) {
                        ...
                        drawdot(uv - 1, inc_y_offset, cy_x, value_floor - ((value_data * eeg_resolution) * .0128),
                            myColor);
                ...
                if (manualControl) {
                    ...
                    if (formatType == 0) {
                        ...
                        for (arr_element in sensorNAME[device_name]) {
                            ...
                            if (init_baseline == true) {
                                baseline[select_contact] =
                                    baseline[select_contact] + parseFloat(contact[select_contact]) + 4201.02564096001;
            ...
            for (var i = 0; i < 32; i++) {
                if (baseline[i] != null) {
                    ...
                    if (reset_counter > 2.9) {
                        //baseline[select_gyro] = baseline[select_gyro] + (cartesian((parseInt(contact[select_gyro])))
                        // * .02);
                    }
                    if (i > 6 && i < 10) {
