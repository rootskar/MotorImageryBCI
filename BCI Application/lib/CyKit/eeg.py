            ...
            if ioCommand[1] == "RecordStart":
                ...
                if os.path.exists(cyPath + "/EEG-Logs") == False:
                    try:
                        ...
                        mirror.text(
                            "*** Failed to Create Directory: '" + cyPath + "/EEG-Logs/' \r\n Please Check Permissions. ")
                        mirror.text(str(msg))
                        return
...
class EEG(object):
    ...
    def __init__(self, model, io, config):
        ...
        self.recordInc = 1
        self.thread_1 = threading.Thread(name='eegThread', target=self.run,
                                         kwargs={'key': self.myKey, 'cyIO': self.cyIO}, daemon=False)
        ...
        self.blank_data = {}
        # BlankData for Epoc??
        self.blank_data[2] = [0, 11, 45, 226, 13, 209, 11, 156, 77, 16, 118, 83, 208, 255, 75, 10, 40, 241, 206, 231,
                              146, 226, 59, 124, 165, 69, 24, 248, 163, 55, 25, 133, 167]
        self.blank_data[6] = [0, 16, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0, 0, 0, 128, 0, 128, 0,
                            ...
                            if useDevice.upper() == "Y":
                                ...
                                if threadMax < 2:
                                    ...
                                    while detail_info == None:
                                        ...
                                        device_firmware = "0x" + str(hex(detail_info[3]))[2:] + str(
                                            hex(detail_info[4])[2:])
                                    if eval(self.cyIO.getInfo("verbose")) == True:
                                ...
                                if all_devices.product_name == "EPOC+":
                                    deviceList[
                                        1] = "empty"  # Direct USB Connection detected. Replace EEG Signals to avoid connecting.
                            ...
                            if useDevice.upper() == "Y":
                                ...
                                if threadMax < 2:
                                    ...
                                    while detail_info == None:
                                        ...
                                        if detail_info == None:
                                            continue
                                        device_firmware = "0x" + str(hex(detail_info[2]))[2:] + str(
                                            hex(detail_info[3])[2:])
                                        software_firmware = "0x" + str(hex(detail_info[4]))[2:] + str(
                                            hex(detail_info[5])[2:])
                                    ...
                                    self.serial_number = str(
                                        cyPyUSB.util.get_string(self.device, self.device.iSerialNumber))
                                    self.product_name = str(
                                        cyPyUSB.util.get_string(self.device, select_device.iProduct))
                                if eval(self.cyIO.getInfo("verbose")) == True:
            ...
            while not tasks.empty() and self.running == True:
                ...
                try:
                        ...
                        if self.format < 1:
                            for i in range(0, 14):
                                packet_data = packet_data + str(
                                    self.convertEPOC(data[1:], self.mask[i])) + self.delimiter
                    ...
                    if self.KeyModel == 4 or self.KeyModel == 7:
                        ...
                        if self.format == 2:
                            ...
                            for i in range(2, len(self.insight_1), 2):
                                ...
                                packet_data = packet_data + str(int(eval(v_1))) + self.delimiter + str(
                                    int(eval(v_2))) + self.delimiter
                        ...
                        if self.format < 1:
                            for i in range(1, 16, 2):
                                ...
                                packet_data = packet_data + str(
                                    self.convertEPOC_PLUS(str(data[i]), str(data[i + 1]))) + self.delimiter
                            ...
                            if self.nobattery == False:
                                packet_data = packet_data + self.delimiter + str(data[16]) + str(self.delimiter) + str(
                                    data[17])
                    ...
                    if self.KeyModel == 6 or self.KeyModel == 5:
                        ...
                        if self.format == 1:
                            for i in range(2, 16, 2):
                                packet_data = packet_data + str(data[i]) + self.delimiter + str(
                            ...
                            if self.nobattery == False:
                                packet_data = packet_data + self.delimiter + str(data[16]) + self.delimiter + str(
                                    data[17])
