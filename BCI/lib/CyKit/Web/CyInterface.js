...
var headset = {
    0: 'epoc', 1: 'epoc', 2: 'epoc', 3: 'insight', 4: 'insight', 5: 'epoc_plus', 6: 'epoc_plus', 7: 'epoc_plus',
    8: 'epoc_flex'
}
...
var qualityDIV = {
    'F3': 2, 'FC5': 3, 'AF3': 0, 'F7': 1, 'T7': 4, 'P7': 5, 'O1': 6, 'O2': 7, 'P8': 8, 'T8': 9, 'F8': 12, 'AF4': 13,
    'FC6': 10, 'F4': 11
}
...
function create_gyro_sliders() {
    ...
    for (var i = 0; i < 9; i++) {
        ...
        gyroSlider[i].setAttribute("style", "position:absolute; left:" + (location_x + 20) +
            "px; top: 180px; height: 10px;  width: 250px; margin: 0; transform-origin: 75px 75px; transform: rotate(-90deg); background-color:" +
            gyroColor[i] + ";");
        ...
        gyroRange.setAttribute("style",
            "position:absolute; width:6px; opacity: .5; top: 153px; left:" + parseInt(location_x - 12) +
            "px; height:50px; background-color:#ffffff;");
        minRange[i] = 50;

        gyroRange = document.getElementById('gyroPane').appendChild(document.createElement("DIV"));
        gyroRange.setAttribute("id", "b_rangeID." + i);
        gyroRange.setAttribute("style",
            "position:absolute; width:6px; opacity: .5; top: 206px; left:" + parseInt(location_x + 24) +
            "px; height:50px; background-color:#ffffff;");
        maxRange[i] = -50;
        triggered[i] = 0;
    }
...
function check_range(select_slider) {
    if (document.getElementById("RangeTriggers").checked == false) {
        return;
    }
...
function create_quality(contact_length, headset_type) {
    ...
    for (var i = 0; i < contact_length; i++) {
        ...
        qualityData[i].innerHTML =
            "<div style='position:relative; padding: 0px 4px; top: -2px; width:15px; padding-top: 10px; height:" + 7 +
            "%; left: 2px; min-width:20px;'><div style='position:absolute; align=right; left:9px; top:15px; z-index:1; font-size:9px; color:white;'> " +
            sensorNAME[headset_type][i] + "</div><div id='qualityID." + sensorNAME[headset_type][i] +
            "' class='tooltip' style='min-height:15px; border-radius:15px; border: 12px solid #333;'><span class='tooltiptext' style='position:absolute; width:30px; left:45px;'>" +
            sensorNAME[headset_type][i] + " </span></div></div>";
...
document.getElementById('maskChange').onclick = function (e) {
    client.sendData("CyKITv2:::setMask:::" + document.getElementById('CyMask').value + ":::" +
        document.getElementById('newMask').value);
    reset_baseline = true;
}
...
function clickGyroRange(e) {
    ...
    for (var i = 0; i < 9; i++) {
        ...
        if (select_range_a != null) {
            ...
            if (e.pageX >
                parseInt(select_range_a.offsetLeft + -20 + parseInt(document.getElementById("gyroPane").offsetLeft))
                && e.pageX <
                parseInt(select_range_a.offsetLeft + 25 + parseInt(document.getElementById("gyroPane").offsetLeft))) {
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    client.onData = function (text) {
        ...
        if (no_counter == true) {
            contact.unshift("0", (data_mode == 2 ? "32" : "16"));
        }
